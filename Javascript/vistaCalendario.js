...
const ModalController = (() => {
  ...
  function open(course) {
    ...
    const enrollButtonText = inCart ? '¡Agregado!' : 'Inscribirse';
    const enrollButtonClass = inCart ? 'added' : 'enroll';
    const enrollButtonDisabled = inCart ? 'disabled' : '';
    ...
    modalContent.innerHTML = `
      ...
      <div class="day-modal-body">
        ${
          course.imageURL
            ? `<img src="${course.imageURL}" alt="${title}" style="max-width: 100px; height: auto; float: right; margin: 0 0 1em 1em; border-radius: 0.3em;">`
            : ''
        }
        <h2>${title}</h2>
...
const CalendarGenerator = (() => {
  ...
  function isCourseInThePast(courseDate) {
    ...
    // Comparamos sólo fecha (si querés más fino, normalizás horas)
  ...
  function render() {
    ...
    const monthText = monthName.charAt(0).toUpperCase() + monthName.slice(1);
    calendarTitle.textContent = `Calendario - ${monthText} ${currentYear}`;
    ...
    // Rellenar hasta 42 celdas (6 filas x 7 columnas)
