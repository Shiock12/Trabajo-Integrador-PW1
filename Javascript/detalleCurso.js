    ...
    const itemsHTML = module.items
      .map((item, iIdx) => {
        ...
        return `
          <li>
            <span class="icono">
              <i class='bx ${item.icon || "bx-book"}'></i>
            </span>
            <div class="info">
...
function renderRelatedCourses(currentCourseId) {
  ...
  wrapper
    .querySelectorAll(".add-to-cart-related[data-course-id]")
    .forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        if (
          ...
          if (course && isCourseExpired(course)) {
...
function setupEnrollment(cursoData) {
  ...
  const applyState = (state, disabled) => {
  ...
  const updateButton = () => {
    if (isCourseExpired(cursoData)) {
      applyState(states.expired, true);
    } else if (isCoursePurchased(cursoData.id)) {
      // ya comprado: dejamos hacer clic para mostrar mensaje
      applyState(states.purchased, false);
    } else if (isCourseInCart(cursoData.id)) {
      // en carrito: también se puede hacer clic para ver modal
      applyState(states.inCart, false);
    } else {
      applyState(states.default, false);
    }
  };
...
function setupLikeButton(cursoData) {
  ...
  likeButton.style.cssText = `
    ...
    font-weight:540;transition:all 0.2s ease-out;
  `;
  ...
  const updateButtonState = () => {
    if (isLiked) {
      likeButton.innerHTML = "❤ Guardado";
      likeButton.style.backgroundColor = "#e6f7ea";
      likeButton.style.color = "#1e6633";
      likeButton.style.borderColor = "#b7e4c7";
      likeButton.title = "Quitar de favoritos";
    } else {
      likeButton.innerHTML = "🤍 Guardar";
      likeButton.style.backgroundColor = "#fff";
      likeButton.style.color = "#333";
      likeButton.style.borderColor = "#ddd";
      likeButton.title = "Añadir a favoritos";
    }
  };

  likeButton.addEventListener("click", () => {
    isLiked = !isLiked;
    if (isLiked) likeCourse(cursoData.id);
    else unlikeCourse(cursoData.id);
    updateButtonState();
  });

  likeButton.addEventListener("mouseenter", () => {
    likeButton.style.transform = "translateY(-1px)";
    likeButton.style.boxShadow = "0 4px 8px rgba(0,0,0,0.05)";
  });
  ...
  container.appendChild(btnInscripcion);
...
function setupModalAndAccordion() {
  ...
  document
    .querySelectorAll(".cerrar-modal, #btn-cerrar-modal")
    .forEach((btn) => btn.addEventListener("click", cerrarModal));
  ...
  document.querySelectorAll(".contenidos details").forEach((detail) => {
    detail.addEventListener("toggle", () => {
      if (detail.open) {
        document.querySelectorAll(".contenidos details").forEach((d) => {
...
function initDetalleCurso() {
  ...
  if (!cursoData) {
    ...
    if (main) {
      main.innerHTML =
        '<h1 style="text-align:center;color:#d32f2f;margin:2em;">❌ Curso no encontrado.</h1>' +
        '<p style="text-align:center;">Verifique el enlace.</p>';
    }
    return null;
  }
  ...
  if (img && cursoData.imageURL) {
    img.src = cursoData.imageURL;
    img.alt = `Imagen para ${cursoData.title}`;
  }

  const h1 = document.querySelector(".curso-detalles h1");
  if (h1) h1.textContent = cursoData.title;

  const p2 = document.querySelector(".curso-detalles p:nth-child(2)");
  const p3 = document.querySelector(".curso-detalles p:nth-child(3)");
  const p4 = document.querySelector(".curso-detalles p:nth-child(4)");
  const p5 = document.querySelector(".curso-detalles p:nth-child(5)");

  if (p2) {
    p2.innerHTML =
      "<strong>Valor:</strong> $" +
      cursoData.valor.toLocaleString("es-AR");
  }
  if (p3) {
    p3.innerHTML = `<strong>Tiempo de dedicación:</strong> ${
      cursoData.dedicacion || "No especificado"
    }`;
  }
  if (p4) {
    p4.innerHTML = `<strong>Descripción:</strong> ${cursoData.description}`;
  }
  if (p5) {
    p5.innerHTML = `<strong>Requisitos Previos:</strong> ${
      cursoData.requisitos || "No especificados"
    }`;
  }
